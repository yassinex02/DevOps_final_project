    ...
    try:
        ...
        for dataset_name, dataset in datasets.items():
            with tempfile.NamedTemporaryFile("w", delete=False) as tmp:
                ...
                logger.info(
                    "Logging %s dataset as a W&B artifact.", dataset_name)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--input", type=str, required=True, help="Input artifact name to split"
    )
    parser.add_argument(
        "--target_column",
        type=str,
    ...
    parser.add_argument(
        "--random_state", type=int, default=42, help="Seed for random number generator"
    )
