"""
This module contains the unit tests for the explore_data module.
"""
...
def profile_report_mock():
    """
    Fixture that returns a mock object for profile report.
    """
...
def test_generate_profiling_report_success(sample_dataframe, profile_report_mock):
    ...
    """
    with patch(
        "src.exploration.explore_data.ProfileReport", return_value=profile_report_mock
    ):
        generate_profiling_report(
            sample_dataframe, "Test Report", "test_report.html", ["col1", "col2"]
        )
        assert profile_report_mock.to_file.called


def test_generate_profiling_report_with_selected_columns(
    sample_dataframe, profile_report_mock
):
    """
...
def test_generate_profiling_report_missing_columns(sample_dataframe):
    ...
    with pytest.raises(ValueError) as excinfo:
        generate_profiling_report(
            sample_dataframe, "Test Report", "test_report.html", ["col1", "col3"]
        )
...
def test_generate_profiling_report_exception(sample_dataframe):
    ...
    """
    with patch(
        "src.exploration.explore_data.ProfileReport",
