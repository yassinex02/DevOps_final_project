...
from src.models.train_model import (evaluate_model, perform_baselining,
                                    perform_model_building, plot_roc_curve,
                                    random_undersampling, save_model,
                                    split_data, train_logistic_regression)
...
@patch('src.models.train_model.evaluate_model', return_value=pd.DataFrame({'Accuracy': [0.5]}))
@patch('src.models.train_model.train_logistic_regression', return_value=LogisticRegression())
@patch('src.models.train_model.perform_baselining', return_value=pd.DataFrame({'Majority Class': [0.5]}))
@patch('src.models.train_model.random_undersampling')
@patch('src.models.train_model.split_data')
def test_perform_model_building(mock_split_data, mock_random_undersampling, mock_perform_baselining,
                                mock_train_logistic_regression, mock_evaluate_model, mock_plot_roc_curve,
                                mock_save_model, dummy_data):
    # Mocking the split data function to return the dummy data directly
    mock_split_data.return_value = (dummy_data.drop('target', axis=1), 
                                    dummy_data.drop('target', axis=1), 
                                    dummy_data['target'], 
                                    dummy_data['target'])
    mock_random_undersampling.return_value = (pd.DataFrame({'feature1': [0, 1], 'feature2': [1, 0]}), pd.Series([0, 1]))
